...
def build_dataset(dataset, dataset_directory, map_path, keywords, debug=False):
    comd="python build.py -raw " + dataset + " -root " + dataset_directory + " -map_path " + map_path + " -name " + dataset + " -min_src_nsents 0 -max_src_nsents 10 -min_src_ntokens_per_sent 0 -min_tgt_ntokens 0 -qf -keywords " + keywords + " -overwrite"
    '''return subprocess.run(["python", 
                        ...
                        "-min_tgt_ntokens", "0",
                        "-qf", "-keywords", (keywords), '-overwrite'], shell=True, stdout=subprocess.PIPE if debug else subprocess.DEVNULL, stderr=subprocess.PIPE if debug else subprocess.DEVNULL)'''
    return os.system(comd)
def do_inference(model, name, binary_data_directory, results_directory, debug=False):
    ...
    command="python train.py -task ext -mode test -batch_size 1 -test_batch_size 1 -bert_data_path " +binary_data_directory+"/t -log_file ../logs/temp -sep_optim true -use_interval true -visible_gpus 0 -max_pos 512 -max_length 200 -alpha 0.95 -min_length 50 -result_path " + result_path + " -test_from " + model
    ...
    '''s = subprocess.run(["python", "train.py", 
                        ...
                        "-test_from", model], shell=True, stdout=subprocess.PIPE if debug else subprocess.DEVNULL, stderr=subprocess.PIPE if debug else subprocess.DEVNULL)'''
    os.system(command)
    return result_path
...
if __name__ == '__main__':
    ...
    if args.model.lower() == 'ctrlsum':
        ...
        original_path = '/content/aosumm/original_text.txt'
        ...
        file_path = '/content/aosumm/result.txt'
